Chapter 2 One-Sample Proportions
================================
...
or the result from a health intervention (success or failure).

Dichotomous categorical data are typically described in terms of the proportion (p) of some population with one of the two possible characteristics.
...
The Complement Rule: The outcomes for dichotomous data must also be
...
pm = 1 − pf = 1 − 0.525 = 0.475.

As a final note on proportions, there is a one-to-one relationship between
...
The key problem here is that we generally do not know the exact value
...
similar situation – must make a decision, so something else must be done.

Thus enters the statistical method and the formation of a hypothesis.
...
As mentioned in Chapter 1, we will create two mutually exclusive hypotheses, such that only one can be true at a time. We name these hypotheses
...
identify the operative phrase “is less than”, which is stating that p < 0.3.

The second step is to find the functional opposite of the statement from
...
the symbolic form of our research question (p < 0.3) is p ≥ 0.3.

The third step is to identify which of our two symbolic forms is the null
...
as well as key words to help in guiding you to the appropriate pair.

2.3 Summarizing Categorical Data (with R Code)

Sample Proportion: Given a set of hypotheses about a population proportion,
the next step is to collect evidence that will (hopefully) support one of the
two hypotheses. When we are interested in a population proportion, the

Table 2.1: Possible Sets of Hypotheses for a Population Proportion Based
...
Hypothesis “at least” “at most”
Null H0 : p = p0 H0 : p = p0 H0 : p = p0
Alternative HA : p<p0 HA : p>p0 HA : p = p0

logical step would be to calculate a sample proportion from a representative
...
of male physicians (ˆpm = 1 − 0.52 = 0.48).

Rounding: Depending on the sample size, you will have different rules for
...
last rule, and 20 was selected for presentation).

2.4 Assessing Assumptions

If our random sample is adequately representative of the parent population
...
adequate sample size rather than ˆp.

2.5 Hypothesis Test for Comparing a Population Proportion to a Hypothesized Value

At this point we have already translated our research question into testable
...
While we cannot answer these questions based on our particular sample,
...
Returning to our example of 200 hospital patients, maybe the historical
...
chips; this is not what we really do, but the idea is the same).

The results from a simulation study that generated 1,000 such samples are
...
our sample proportion could change in value if we were to resample.

Note that a frequency of 20 occurs most often (∼12% of the time) and
...
proportions close to 0.10 rather than far away from it.

2.5.2 Decision Making

So how likely is our sample value of 33? Based on our simulated data, 33
...
This last statement brings us to the crux of statistical decision making:
...
the “truth” and our assumption was wrong.

Since our sample is the only information we have that reflects any property
...
likely to be true (or at least more likely than some alternative).

Returning to our example, since a frequency of 33 (or greater) did not
...
is probably greater than 0.10.

2.5.3 Standard Normal Distribution

While the previously conducted simulation study was helpful in discussing
...
Consider the histogram in Figure 2.1, which shows in graphical form the
...
values that are unlikely to occur if our assumption of p = 0.10 is true.

As mentioned earlier, we do not want to rely upon simulation studies or
...
The standard normal distribution is centered at zero with a variance of one
...
To show how the normal distribution works, we have reproduced the
...
our assumptions are met, we should feel comfortable using the normal distribution to represent the distribution of our sample estimate.

2.6 Performing the Test and Decision Making (with R Code)

2.6.1 Test Statistic

So while we can use the standard normal distribution to answer probabilistic statements about our data and hypotheses about the population 
...
To do this, note that the distribution of ˆp in Figure 2.1 is centered near the
...
since it contains negative proportions), which is centered at zero.

While the distribution is now centered correctly, it still requires a variance
...
Since the statistic z – known as a test statistic – has a standard normal
...
null or alternative) the data supports.

Returning to our example, recall that x = 33, n = 200, ˆp = 0.165, and
p0 = 0.10. Thus, our test statistics is
...
which means that the sample proportion 0.165 is slightly more than three
...
z – to two decimal places, meaning we would report z = 3.06.

Unfortunately, R does not compute the test statistic just provided.
However, the R function prop.test() does provide an equivalent (though
cosmetically different) test for proportions, the syntax for which is as follows:
...
Notice that R provides a considerable amount of output (some of which
...
relationship will be explained further in the next Chapter.

2.7 Formal Decision Making

2.7.1 Critical Value Method

The most traditional method of making a decision in a hypothesis test is to
...
and compare it to the critical value to make our decision.

Finding the critical value depends upon our alternative hypothesis and
...
order for us to reject the null hypothesis in favor of the alternative.

Given a specified significance level (and α = 0.05 is generally used), the
...
select the null or alternative hypothesis.

In our example, say our original research statement was: the proportion
...
discuss what this means and how we react later.
...
of our observed test statistic (|z|) and less than or equal to the negative of
the absolute value of our observed test statistic (−|z|).

The third part is calculating the desired probability, which of course
...
packages (including R; see Program 2 above).

Regardless of the method of computation, the probability being calculated
...
probability that Z is greater than |z| (or 2P(Z > |z| |H0)), or we calculate
two-times the probability that Z is less than −|z| (or 2P(Z < −|z| |H0)).
Admittedly, these definitions are complicated, but the good news is that you
will not have to calculate them by hand.

To put the p-value into practice, we must compare it to the stated significance level α. In order to reject the null hypothesis, we would need an
...
our p-value is greater than or equal to the significance level (p-value ≥ α).

For our example, our observed test statistic (z = 3.06) and the right-tailed hypothesis test means that our p-value is 0.001092. This is less than 
...
p-values to at most four decimal places, so we should report p-value = 0.0011.

2.7.3 Conclusion

Whether we used the critical value or p-value method, we report our results
...
In our example, our test statistic fell in the rejection region (the p-value
...
evidence, which is not the same as definitive proof.

The R software conducts the two-sided test (HA : p = p0) by default,
...
default to the "two.sided" case and will perform the two-sided test.
...
For the right-tailed hypothesis in our example (HA : p > 0.10) we would
...
what we reported for the z-test results.

2.7.4 Confidence Intervals

The sample proportion ˆp is dependent upon the sample we collect and the
...
with a confidence interval that takes into account sampling variability.

A confidence interval is straight forward to calculate, though somewhat
...
To calculate a (1 − α)% confidence interval (or CI) we need three things:
...
point estimate. With these three pieces, our CI would take the form:

(Point Estimate ± Measure of Variability × Probabilistic Measure)

The ± sign indicates that by adding and subtracting the second part from
...
(0.165 − 1.96 × 0.026, 0.165 + 1.96 × 0.026) = (0.114, 0.216)

To interpret this interval, we would say “a 95% confidence interval of the
...
Note that we could have used R to produce this confidence interval, but
...
the confidence interval we obtained by hand.
...
hand when the sample sizes are small enough.

For instance, imagine the case where someone gives you a cup and tells
...
not enough evidence for us to reject the null hypothesis.

Now let’s assume that we have two cups, where the first is filled with
...
our p-value (0.25) would still not lead us to reject the null hypothesis.

If we have three cups (filled with Pepsi, Coke and Coke, respectively),
...
While we will not enumerate the outcomes, Table 2.7 presents the outcomes from both four-cup and five-cup experiments. Here there is still only
...
In each of these cases, the number of correct guesses follows a binomial distribution, where the probability of a given number of correct guesses depends
...
is 0.10 in favor of the alternative that the population success rate is larger.

We use the binom.test() function to calculate the exact binomial test
in R. The general syntax is similar to the test on proportions using the
prop.test() function and is given by:
...
where x is the number of successes, n is the number of trials, p0 is the
...
Note in Program 3 that the output is similar to that of prop.test(). Here
...
The following write-up is an example of the material that we need to communicate if we had actually conducted the example study used for the majority
...
little to no adverse side-effects was greater than 0.10.

Methods: The frequency of subjects reporting reduced side-effects from
...
alternative hypothesis if the p-value is less than α; otherwise we will not reject the null hypothesis. The R statistical software was used for all analyses.

Results: Assuming that the sample was representative and subjects were
...
treatment-related side-effects may wish to consider this treatment.
...
i. If np0 > 5 and n(1 − p0) > 5, then use z-test or chi-square
test (in R).

ii. If np0 < 5 or n(1 − p0) < 5, then use binomial test.
...
1. A researcher is interested in the proportion of active duty police officers who pass the standard end of training fitness test. They take a
...
on the police force that can pass the fitness test.

2. Occupational health researchers are interested in the health effects of
...
3. In December 2012 Gallup Poll conducted a survey of 1,015 Americans
...
4. Norman et al. (2013) Consider the preference for walkability of neigh-borhoods for obese men. They studied 240 obese 
...
confidence interval for the proportion of obese men who walk for transportation.

5. Barrison et al. (2001) are interested in the reasons that proton pump
...
interval for the proportion of all gastroenterologists who prescribe proton pump inhibitors.

6. Barrison et al. (2001) were interested in the proportion of physicians
...
7. Keightley et al. (2011) are concerned with obese peoples self perceptions. They hypothesize that a majority of obese people can identify
...
test to determine whether or not their hypothesis is warranted.

8. Salerno et al. (2013) is interested in determining the current infection
...
all high school students who have one of the two diseases.
